    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-6">
            ...
            @if(auth()->user()->user_type === 'team_member')
                <!-- Team Member: Only Status Update -->
                <div>
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 space-y-3">
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Task</label>
                        <p class="text-sm text-gray-900 dark:text-white mt-1">{{ $task->title }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Project</label>
                        <p class="text-sm text-gray-900 dark:text-white mt-1">{{ $task->project->title }}</p>
                    </div>
                    @if($task->description)
                    <div>
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Description</label>
                        <p class="text-sm text-gray-900 dark:text-white mt-1">{{ $task->description }}</p>
                    </div>
                    @endif
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Priority</label>
                            <p class="text-sm text-gray-900 dark:text-white mt-1">{{ ucfirst($task->priority) }}</p>
                        </div>
                        @if($task->due_date)
                        <div>
                            <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Due Date</label>
                            <p class="text-sm text-gray-900 dark:text-white mt-1">{{ $task->due_date->format('M d, Y') }}</p>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            @else
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $task->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
